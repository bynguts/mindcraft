...
const hellsKitchenProgressManager = {
    ...
    readProgress: function (taskId) {
        const file = this.getProgressFile(taskId);
        try {
            if (existsSync(file)) {
        ...
        return { taskId: taskId, agent0Complete: false, agent1Complete: false };
    },

    writeProgress: function (taskId, progress) {
        const file = this.getProgressFile(taskId);
        try {
            writeFileSync(file, JSON.stringify(progress), 'utf8');
        } catch (err) {
